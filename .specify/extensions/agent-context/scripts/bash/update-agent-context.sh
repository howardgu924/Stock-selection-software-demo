...
trap 'rm -f "$TMP_SECTION"' EXIT
{
  ...
  echo ""
  echo "Spec Kit is the source of truth for feature work. Vague change requests"
  echo "must not be treated as complete specifications. If a request says to"
  echo "\"modify\", \"optimize\", \"adjust\", \"improve\", or \"refactor\" behavior without"
  echo "defining the desired target behavior, mark the spec as NEEDS CLARIFICATION"
  echo "and ask questions before planning."
  echo ""
  echo "Do not invent product rules, formulas, filters, ranking/scoring logic,"
  echo "thresholds, data periods, output fields, UI behavior, or acceptance"
  echo "criteria. Do not proceed to plan/tasks/implementation while the spec has"
  echo "unresolved product decisions."
  echo ""
  echo "When Superpowers is used, use it as a readiness, TDD, debugging, code"
  echo "review, and verification quality gate. It must review whether the current"
  echo "spec/plan/tasks are sufficient for the next phase, not merely whether the"
  echo "text paraphrases the user request."
  echo "$MARKER_END"
} > "$TMP_SECTION"
