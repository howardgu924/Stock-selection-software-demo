...
$lines += ''
$lines += 'Spec Kit is the source of truth for feature work. Vague change requests'
$lines += 'must not be treated as complete specifications. If a request says to'
$lines += '"modify", "optimize", "adjust", "improve", or "refactor" behavior without'
$lines += 'defining the desired target behavior, mark the spec as NEEDS CLARIFICATION'
$lines += 'and ask questions before planning.'
$lines += ''
$lines += 'Do not invent product rules, formulas, filters, ranking/scoring logic,'
$lines += 'thresholds, data periods, output fields, UI behavior, or acceptance'
$lines += 'criteria. Do not proceed to plan/tasks/implementation while the spec has'
$lines += 'unresolved product decisions.'
$lines += ''
$lines += 'When Superpowers is used, use it as a readiness, TDD, debugging, code'
$lines += 'review, and verification quality gate. It must review whether the current'
$lines += 'spec/plan/tasks are sufficient for the next phase, not merely whether the'
$lines += 'text paraphrases the user request.'
$lines += $MarkerEnd
